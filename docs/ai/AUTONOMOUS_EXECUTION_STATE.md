...
- 当前提交：`e7e084c`（最终证据提交前的交付实现 HEAD）
...
- 最终 ZIP：`dist/ReadonlyBusinessDbTool-v0.5.0-rc.1.zip`，大小 `31,090,050` 字节。
- SHA-256：`6DE117A0034B8887930735AC138C9253CB11C1C673A4C6C214B78AE85BF1672F`；与 `dist/SHA256SUMS.txt` 独立重算一致。
- 最终 ZIP 解压到 `build/final-verification-20260714-042258/中文 现场 交付/ShelfPointMonitor`；临时移除系统 Java PATH 后，三个 Java 入口与三个中文脚本均退出 0。
- 诊断 ZIP 六项白名单精确匹配，敏感模式命中 0。
- 发布 ZIP 共 180 项，必需文件齐全，禁止条目 0，项目自有文本敏感模式命中 0。
- 内嵌 runtime 自带两处标准回环地址 `127.0.0.1`；已从项目自有文本边界中区分记录，不属于现场信息。
